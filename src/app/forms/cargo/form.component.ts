...
export class CargoFormComponent {
  ...
  load() {
    this.loading = true;
    this.api.list<any>('cargo').subscribe({
  ...
  edit(row: any) {
    this.model = { ...row };
    const idGuess = row.id ?? row.id_cargo ?? row.id_cargo ?? row.id_persona ?? row.id_cargo ?? row.id_insumo ?? row.id_examen ?? row.id_pago ?? row.id_cita ?? row.id_fecha ?? row.id_registro ?? row.idMunicipio ?? row.idDepartamento ?? row.id_rol;
  ...
  save() {
    ...
    this.loading = true;
    const obs = id ? this.api.update<any>('cargo', id, body) : this.api.create<any>('cargo', body);
    obs.subscribe({
